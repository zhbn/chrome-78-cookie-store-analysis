      ...
      ;创建了一个加密解密的对象，后面是如何传递到cookie读取保存时用于加密解密呢，再这个文件中继续向下分析，可以看到  
       ...
       >>case PendingOperation::COOKIE_ADD:  
                 ...
                 >>>>if (crypto_ && crypto_->ShouldEncrypt()) {  
                   ...
                   >>>>>>>>add_smt.BindBlob(4, encrypted_value.data(), static_cast<int>(encrypted_value.length()));  
                 >>>>>>} else {  
                 ...
                 >>>>>>add_smt.BindInt(  9, CookieSameSiteToDBCookieSameSite(po->cc().SameSite()));  
                ...
                >>>>>> add_smt.BindInt(11, po->cc().IsPersistent());  
                 ...
                 >>>>>>add_smt.BindInt(  13, CookiePriorityToDBCookiePriority(po->cc().Priority()));  
                 >>>>>>if (!add_smt.Run()) {  
